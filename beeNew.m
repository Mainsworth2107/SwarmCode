%% ABC+ Solution Shifitng

function sol = beeNew(A,i,neighbour)
    test = A(i,:);
    
    % Getting global object postions
    global allObjs;
    objs = height(allObjs);
    
    % Shorthand for currnet and neigbouring solution
    P = A(i,:);
    Q = A(neighbour,:);
    
    %Finds points of equality between current and neigbouring solution to direct shifitng
    Equal = find(P==Q); 
    
    %% Swapping wrt to neighbour
    
    % A swap can eihter move towards or away from a neigbouring soluiton
    ...
    if( 2 < length(Equal) )&& ( length(Equal) < (length(P) - 2) ) 
        action = round(rand); %If so, picck a random motion direction
    else
        if(length(Equal)<2) % If all solution variables are differnt, move towards
            action = 1;
    ...
    while P(R(idxs(1))) == P(R(idxs(2)))
       ...
       cut = cut + 1;
       %Ensure abnormal swap sets dont freeze the code
       if(cut > 100)
           break;
       end
    end

    %Provided a valid set of idexs has been found, perfome swapping
    if(cut < 100)
    ...
    if(rand <= mutation) %In the case a solution is muatated,
       mutation = 0.1;
       ...
       for i=1:length(sol) 
           if(rand < mutation)
               ...
               if(sol(i) == 0) %As MATLAB indexs from 1.
                   sol(i) = sol(i) + objs;
               end
           end
       end
